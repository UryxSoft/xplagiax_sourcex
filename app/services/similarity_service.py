...
class SimilarityService:
    """Service for similarity search and plagiarism detection"""
    ...
    async def search_similarity(
        ...
        """
        logger.info(
            f"Starting similarity search: theme='{theme}', texts={len(texts)}, threshold={threshold}"
        )
        ...
        for idx, (page, paragraph, text) in enumerate(texts):
            ...
            # 6. Calculate similarity with embeddings
            ...
            if matches:
                await self.cache.save_to_cache(
                    cache_key,
                    [vars(m) for m in matches]
                )
            
            all_results.extend(matches)
        
        logger.info(f"Similarity search completed: {len(all_results)} results found")
    ...
    async def check_plagiarism(
        ...
        for result in results:
            by_level[result.plagiarism_level].append(vars(result))
    ...
    async def _calculate_similarities(
        ...
        query_embedding = self.embedding_service.encode([processed_text])[0]
